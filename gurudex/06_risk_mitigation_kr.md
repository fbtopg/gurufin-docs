...
flowchart TD
    ...
    subgraph Layer3["Layer 3: 제한 및 검증"]
        ...
        L3B["오라클 3중 검증:<br/>• 시간 < 5분<br/>• 편차 < 1%<br/>• 신뢰도 > 95%"]
        L3C["기관별<br/>일일 한도"]
        L3A --> L3B --> L3C
    end
...
\[ Fee\_{adjusted} = Fee\_{base} \times (1 + Coefficient\_{utilization} + Coefficient\_{volatility}) ]

**구성 요소**:

* **Fee\_base**: 기본 수수료 (예: 0.3%)
* **Coefficient\_utilization**: 풀 사용률 계수 (유동성 부족도)
* **Coefficient\_volatility**: 환율 변동성 계수 (시장 불안정도)
...
* **기준**: 이전 가격 대비 1% 이내의 변동만 허용 (기본값, 최대 5%까지 설정 가능)
...
GuruDex는 **풀 유동성 상태에 따른 4단계 서킷 브레이커**를 운영하도록 설계되었습니다:

| 레벨          | 명칭                                                         | 유동성 상태 | 자동 조치                                                                | 영향 범위                             |
| ----------- | ---------------------------------------------------------- | ------ | -------------------------------------------------------------------- | --------------------------------- |
| **Level 1** | <p><strong>HEALTHY</strong><br><strong>(정상)</strong></p>   | 80% 이상 | <p>- 정상 운영<br>- 표준 수수료 적용</p>                                        | 모든 거래 정상 작동                       |
| **Level 2** | <p><strong>WARNING</strong><br><strong>(경고)</strong></p>   | 60-80% | <p>- 동적 수수료 증가 시작<br>- 블록당 스왑 횟수 제한<br>- 모니터링 강화</p>                 | 대규모 거래 일부 제한                      |
| **Level 3** | <p><strong>CRITICAL</strong><br><strong>(위험)</strong></p>  | 40-60% | <p>- 동적 수수료 대폭 인상<br>- 거래당 최대 금액 제한 강화<br>- 긴급 유동성 요청</p>            | <p>스왑 제한 강화<br>일부 거래 차단 가능</p>    |
| **Level 4** | <p><strong>EMERGENCY</strong><br><strong>(긴급)</strong></p> | 40% 미만 | <p>- 모든 신규 스왑 중단<br>- 긴급 유동성 공급 실행<br>- RFQ 활성화<br>- 풀 간 자동 리밸런싱</p> | <p>거래 전면 중단<br>Operator 개입 필수</p> |
...
사용자는 이러한 포괄적인 위험 관리 기능을 통해 안전하고 신뢰할 수 있으며 탄력적인 FX 스왑 환경에서 거래할 수 있을 것입니다.
