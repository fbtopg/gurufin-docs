...
│   FXSwap 운영팀이 신원 확인 및 자금세탁방지 검증 수행        │
│   - 법인 등록 증명서 확인                                    │
│   - 대표자 신원 확인                                         │
│   - 자금 출처 검증                                           │
│   - 규제 준수 여부 확인                                      │
└─────────────────────────────────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│              Step 3: 기관 활성화 및 파라미터 설정             │
│   Operator → activateInstitution()                          │
│   - 거래 한도 설정 (perTxLimit, dailyLimit)                 │
│   - 맞춤형 수수료율 설정 (customFeeRate)                     │
│   - 최대 가격 편차 설정 (maxPriceDeviation)                 │
│   - 데이터 신선도 요구사항 설정 (maxStaleness)              │
│   Status: ACTIVE                                            │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────┐
│         Step 4: 코인 및 풀 접근 권한 부여                     │
