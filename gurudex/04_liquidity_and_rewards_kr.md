...
2. **가상 LP 토큰 발행**:
   ...
   * 첫 번째 유동성 추가 시: \[ \text{virtualLP} = \sqrt{\text{amount}_{\text{USGX\}} \times \text{amount}_{\text{KRGX\}}} ]
   * 후속 유동성 추가 시: \[ \text{virtualLP} = \min\left(\frac{\text{amount}_{\text{USGX\}}}{\text{reserve}_{\text{USGX\}}}, \frac{\text{amount}_{\text{KRGX\}}}{\text{reserve}_{\text{KRGX\}}}\right) \times \text{totalVirtualLP} ]
3. **지분 관리**:
...
GuruDex는 기본 수수료(Base Fee)와 프로토콜 수수료(Protocol Fee)를 구분하여 부과합니다:

| 사용자 타입     | 기본 수수료    | 프로토콜 수수료 | 총 수수료     |
| ---------- | --------- | -------- | --------- |
| **일반 사용자** | 0.3% (동적) | 0.05%    | **0.35%** |
| **기관 사용자** | 0.1% (고정) | 0.05%    | **0.15%** |

**일반 사용자 수수료 (동적)**:

* Uniswap v3 스타일 AMM 모델을 따르는 풀 불균형 기반 계산
* 수수료율은 기본 및 견적 자산 간의 균형 잡힌 1:1 유동성 비율에서 1% 편차마다 1%씩 증가
* 기본 수수료 0.3%에 프로토콜 수수료 0.05% 추가
* 동적 수수료 메커니즘은 재균형을 장려하고 역선택으로부터 보호

**기관 사용자 수수료 (고정)**:

* 기본 수수료 0.1% + 프로토콜 수수료 0.05% = 총 0.15%
* 실시간 환율 기반 스왑에 적용
* 최소 슬리피지로 대량 거래를 수용하기 위해 낮게 설정
...
GuruDex는 규제 준수를 위해 차등 리워드 분배 시스템을 구현합니다:

* **대부업법**: 연 20% 이자 상한
* **GuruDex 적용**: **보수적으로 연 7% 적용** (안전마진 확보)
* **목적**: 기관 투자자 과도한 수익 방지 + 규제 준수

### 차등 리워드 분배 시스템

| LP 유형     | 리워드 계산 방식              | 예상 수익      |
| --------- | ---------------------- | ---------- |
| **기관 LP** | 법정 허용 이자율 연 7% 상한      | 최대 7%      |
| **일반 LP** | (기관 리워드 - 7%) + 일반 리워드 | **고수익 예상** |

**작동 메커니즘**:

1. 기관 LP가 본래 받을 수수료 중 7%를 초과하는 부분 계산
2. 초과 부분을 일반 LP 풀에 재분배
3. 일반 LP는 자신의 기여분 + 기관 초과 리워드 수령
4. 결과적으로 일반 LP의 수익률이 기관 LP보다 높아짐

**예시**:

* 기관 LP 원래 수익: 10% → 7% 상한 적용 → 3% 초과분 발생
* 일반 LP 원래 수익: 5% → 3% 초과분 추가 → 최종 8% 수익
* 결과: 일반 LP가 더 높은 수익률 달성 (수익 공정성 개선)
...
3. **지분 계산**:
   * 각 LP의 보상 지분은 총 유동성에 대한 LP 토큰 잔액을 기반으로 계산: \[ \text{reward}_{\text{LP\}} = \text{totalFees} \times \frac{\text{lpTokens}_{\text{LP\}}}{\text{totalLPTokens\}} ]
4. **보상 할당**:
